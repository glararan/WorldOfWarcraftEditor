...
void MapViewer::tick(float t, float dt)
{
	...
	if(SDL_GetAppState()&SDL_APPINPUTFOCUS)
	{
		...
		{
			if(moving != 0)
				world->camera += dir * dt * movespd * moving;
			if(strafing != 0)
			{
			...
			if(updown != 0)
				world->camera += Vec3D(0, dt * movespd * updown, 0);
		...
		{
			if(moving != 0)
				world->camera.z -= dt * movespd * moving/(world->zoom * 1.5f);
			if(strafing != 0)
				world->camera.x += dt * movespd * strafing/(world->zoom * 1.5f);
			if(updown != 0)
				world->zoom *= pow(2.0f, dt * updown * 4.0f);
			if(world->zoom > 2)
				world->zoom = 2;
	...
	else if(TileMode)
	{
		...
		if(hud)
		{
			...
			try
			{
				...
				string areaName = rec.getString(AreaDB::Name);
			...
			if(regionID != 0)
			{
				...
				try
				{
					...
					string regionName = rec.getString(AreaDB::Name);
	...
	else
	{
		...
		if(hud)
		{
			...
			if((Selection != 0) && (Selection->type != ENTRY_FAKE))
			{
				...
				f16->shprint(5, video.yres - 40 + TextOffset, "(%.0f, %.0f, %.0f) Tile %.0f %.0f (%.0f, %.0f)", -(world->camera.x - ZEROPOINT), -(world->camera.z - ZEROPOINT),	world->camera.y, (-(TILESIZE/2))/TILESIZE, (world->camera.z-(TILESIZE/2))/TILESIZE, world->camera.x, world->camera.z);
				...
				f16->shprint(5, video.yres - 22, "(%.0f, %.0f, %.0f) Tile %.0f %.0f (%.0f, %.0f)", -(world->camera.x - ZEROPOINT), -(world->camera.z - ZEROPOINT), world->camera.y, (world->camera.x - (TILESIZE/2))/TILESIZE, (world->camera.z - (TILESIZE/2))/TILESIZE, world->camera.x, world->camera.z);
