...
Model::Model(string name, bool forceAnim) : ManagedItem(name), forceAnim(forceAnim)
{
	// replace .MDX with .M2
	char tempname[256];
	...
	if(false || name == "World\\Kalimdor\\Orgrimmar\\Passivedoodads\\Orgrimmarbonfire\\Orgrimmarsmokeemitter.Mdx")
...
int ModelManager::add(string name)
{
	int id;
...
{
	for(map<int, ManagedItem*>::iterator it = items.begin(); it != items.end(); ++it)
...
void ModelInstance::draw()
{
	...
	if(dist > 2 * gWorld->modeldrawdistance)
		return;
...
map<int, ModelEntry, ltint>	ModelList;
...
void drawModelList()
{
	for(map<int, ModelEntry, ltint>::iterator i = ModelList.begin(); i != ModelList.end(); i++)
