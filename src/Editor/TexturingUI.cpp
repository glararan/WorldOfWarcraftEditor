...
map<GLuint, ManagedItem*>::iterator FindCurTexture()
{
	//want to guarantee the texture is there
	video.textures.add(CurTextureName);

	//now find where it is
	for(map<GLuint, ManagedItem*>::iterator i = video.textures.items.begin(); i != video.textures.items.end(); i++)
...
void updateTextures()
{
	map<GLuint, ManagedItem*>::iterator t = FindCurTexture();
	for(int i = 0; i < 8; i++)
	{
CheckForEnd:
