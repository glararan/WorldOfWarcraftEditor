...
{
	if (!gWorld->frustum.intersects(vmin,vmax)) return;
	for (int i=0; i<4; i++) children[i]->drawColor();
}
...
void MapTile::saveTile()
{
	...
	std::map<GLuint,string>	texturesUsed;
	std::map<GLuint,int>	textureOffsets;
	string	AllTextures;
	char *allTextures;
	int		texSize;
	for(i=0;i<256;i++)
	...
	i=0;
	for(std::map<GLuint,string>::iterator texName=texturesUsed.begin();texName!=texturesUsed.end();texName++)
	{
		char wtf[512];
